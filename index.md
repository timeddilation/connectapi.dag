...
under the hood for managing DAG deployments to Posit Connect. Although,
it is not required to use this package’s built-in features for DAG
deployments, as you can do whatever you want with the DAG environment
...
For each of these functions, they may accept any number of tasks to set
as dependencies.
...
function. Simulated tasks created this way will always run successfully
...
function. This function also creates a simulated ConnectTask
environment, but allows you to specify a probability of failure.
...
You may also plot the DAG. By default, the
[`plot()`](https://rdrr.io/r/graphics/plot.default.html) method on the
DAG will create a plotly graph. You may plot the DAG even when the DAG
is invalid. This can be useful when troubleshooting circular references,
...
function handles most of the work for you. The DAG environment has a
`pin_name` attribute, which can be changed with the
...
function to generate the Rmd file needed to execute and schedule the
DAG.
...
will first reset the DAG using
